...
select min(salario) as menor_renda, max(salario) as maior_renda from empregado where sexo='F'; 
select min(salario) as menor_renda, max(salario) as maior_renda from empregado where sexo='M'; 
...
select replace (nome, "A", "") as nome_modificando,
